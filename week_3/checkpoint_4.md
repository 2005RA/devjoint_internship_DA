...
Cards respond dynamically to Year/Month/Category/Channel slicers. A "vs Previous Period" card using `PREVIOUSMONTH()` was attempted but removed since the model's date table setup didn't support it reliably within the time available; target comparison covers the KPI requirement instead.
